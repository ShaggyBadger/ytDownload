...
def _call_gemini(prompt, retries=3, delay=5):
    ...
    """
    logger.debug(f"Attempting to call Gemini CLI with prompt of length {len(prompt)} characters.")
    for attempt in range(retries):
        ...
        try:
            ...
            process = subprocess.run(['gemini'], input=prompt, capture_output=True, text=True, check=True)
            
            # Check for quota error message in stderr, even if process exits successfully
            if "quota" in process.stderr.lower():
                logger.warning(f"Gemini API quota exceeded on attempt {attempt + 1}. STDERR: {process.stderr}")
                raise GeminiQuotaExceededError(f"Gemini API quota exceeded: {process.stderr}")

            logger.debug(f"Gemini call successful on attempt {attempt + 1}. STDOUT: {process.stdout[:100]}...")
            return process.stdout.strip()

        except FileNotFoundError:
            logger.error("CRITICAL: 'gemini' command not found. Make sure the Gemini CLI is installed and in your system's PATH.")
            raise RuntimeError("Gemini CLI not found.")
        except subprocess.CalledProcessError as e:
            logger.error(f"Gemini CLI call failed on attempt {attempt + 1} with exit code {e.returncode}:")
            logger.error(f"STDOUT: {e.stdout}")
            logger.error(f"STDERR: {e.stderr}")
            if "quota" in e.stderr.lower():
                logger.critical(f"Gemini API quota exceeded. Raising GeminiQuotaExceededError. STDERR: {e.stderr}")
            ...
            logger.critical("Caught GeminiQuotaExceededError. Re-raising to halt processing.")
            raise e
        except Exception as e:
            logger.error(f"An unexpected error occurred during Gemini CLI call on attempt {attempt + 1}: {e}", exc_info=True)
            if attempt < retries - 1:
                logger.info(f"Retrying in {delay} seconds...")
                time.sleep(delay)
            else:
...
def _execute_processing_stage(transcript_processing_id, stage_logic_func, success_status, stage_name, current_index=None, total_count=None, stop_processing_flag=None):
    ...
    """
    logger.debug(f"Executing stage '{stage_name}' for transcript_processing_id: {transcript_processing_id}")
    db_session = db.SessionLocal()
    tp = None
    try:
        tp = db_session.query(db.TranscriptProcessing).filter(db.TranscriptProcessing.id == transcript_processing_id).first()
        if not tp:
            logger.error(f"No transcript processing entry found with id: {transcript_processing_id}. Stage '{stage_name}' aborted.")
            return
        ...
        new_status = f"{stage_name.lower().replace(' ', '_')}_{model_name.lower()}_failed"
        logger.critical(f"!!! CRITICAL: {model_name} API processing issue during {stage_name} for transcript {transcript_processing_id}. New status: '{new_status}'. Stopping further LLM processing. Error: {e}", exc_info=True)
        ...
        if tp:
            ...
            db_session.commit()
            logger.info(f"Status updated for transcript {tp.id} after {model_name} API failure.")
        ...
        logger.error(f"An unexpected error occurred during {stage_name} for transcript {transcript_processing_id}. New status: '{new_status}'. Error: {e}", exc_info=True)
        if db_session.is_active:
            db_session.rollback()
            logger.debug("Database session rolled back.")
        if tp:
            ...
            tp.status = new_status
            db_session.commit()
            logger.info(f"Status updated for transcript {tp.id} after unexpected error.")
        # Re-raise to stop current transcript's processing
        raise RuntimeError(f"Processing halted due to unexpected error.") from e
    finally:
        logger.debug(f"Closing database session for stage '{stage_name}', transcript {transcript_processing_id}.")
        db_session.close()
...
def _initial_cleaning_logic(tp, db_session):
    ...
    try:
        ...
        with open(tp.raw_transcript_path, 'r') as f:
            raw_text = f.read()
        logger.debug(f"Successfully read {len(raw_text)} characters from raw transcript.")
        ...
        # Update the database
        logger.debug(f"Updating database with initial_cleaning_path: {initial_cleaning_path}")
        ...
        logger.error(f"File not found during initial cleaning for transcript {tp.id}: {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred in _initial_cleaning_logic for transcript {tp.id}: {e}", exc_info=True)
        raise
...
def _final_pass_logic(tp, db_session):
    ...
    try:
        ...
        logger.debug(f"Reading secondary cleaned transcript from: {tp.secondary_cleaning_path}")
        with open(tp.secondary_cleaning_path, 'r') as f:
            secondary_text = f.read()
        logger.debug(f"Successfully read {len(secondary_text)} characters from secondary cleaned transcript.")
        ...
        # Update the database
        logger.debug(f"Updating database with final_pass_path: {final_pass_path} and final_word_count: {final_word_count}")
        ...
        logger.error(f"File not found during final pass for transcript {tp.id}: {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred in _final_pass_logic for transcript {tp.id}: {e}", exc_info=True)
        raise
...
def _python_scrub_logic(tp, db_session):
    ...
    try:
        ...
        logger.debug(f"Reading initial cleaned transcript from: {tp.initial_cleaning_path}")
        with open(tp.initial_cleaning_path, 'r') as f:
            initial_cleaning_text = f.read()
        logger.debug(f"Successfully read {len(initial_cleaning_text)} characters from initial cleaned transcript.")
        ...
        logger.error(f"File not found during python scrub for transcript {tp.id}: {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred in _python_scrub_logic for transcript {tp.id}: {e}", exc_info=True)
        raise
...
def _llm_book_cleanup_logic(tp, db_session):
    ...
    try:
        ...
        logger.debug(f"Successfully read {len(text_to_clean)} characters from python scrubbed transcript.")
        ...
        for p in paragraphs:
            if len(current_chunk.split()) + len(p.split()) < 1000:
                ...
                current_chunk = p + "\n\n"
        chunks.append(current_chunk)
        logger.info(f"Text for transcript {tp.id} has been split into {len(chunks)} chunks.")

        cleaned_chunks = []
        for i, chunk in enumerate(chunks):
            logger.info(f"Processing chunk {i+1}/{len(chunks)} for transcript {tp.id}...")
            try:
                ...
                logger.info(f"Successfully processed chunk {i+1}/{len(chunks)} for transcript {tp.id}.")
            except RuntimeError as e:
                logger.error(f"Error processing chunk {i+1} for transcript {tp.id}: {e}", exc_info=True)
        ...
        logger.debug(f"Reassembling {len(cleaned_chunks)} cleaned chunks for transcript {tp.id}.")
        final_text = "\n\n".join(cleaned_chunks)
        logger.info(f"Final text for transcript {tp.id} reassembled, length: {len(final_text)} characters.")
        ...
        logger.error(f"File not found during LLM book cleanup for transcript {tp.id}: {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred in _llm_book_cleanup_logic for transcript {tp.id}: {e}", exc_info=True)
        raise
...
def _update_metadata_file_logic(tp, db_session):
    ...
    try:
        if not tp.book_ready_path or not tp.metadata_path:
            logger.error(f"Missing book_ready_path or metadata_path for transcript {tp.id}. Aborting metadata update.")
        ...
        with open(tp.metadata_path, 'r') as f:
            try:
                ...
                logger.warning(f"Could not parse JSON from {tp.metadata_path}. Starting with an empty metadata object.")
                metadata = {}

        # 2. Calculate word count
        logger.debug(f"Reading book-ready text from: {tp.book_ready_path} to calculate final word count.")
        ...
        if duration_str:
            logger.info(f"Retrieved video duration for transcript {tp.id}: {duration_str}")
        else:
            logger.warning(f"Could not retrieve video duration for transcript {tp.id}.")


        # 4. Add new data
        logger.debug(f"Updating metadata object for transcript {tp.id} with final_word_count and trimmed_video_duration.")
        ...
        logger.error(f"File not found during metadata update for transcript {tp.id}: {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"An unexpected error occurred in _update_metadata_file_logic for transcript {tp.id}: {e}", exc_info=True)
        raise
...
def _export_sermon_file_logic(tp, db_session):
    ...
    try:
        ...
        logger.info(f"Delegating sermon export for transcript {tp.id} to sermon_exporter.export_single_sermon.")
        ...
        logger.error(f"An unexpected error occurred during sermon export for transcript {tp.id}: {e}", exc_info=True)
        raise
...
def reset_transcript_status(transcript_id, db_session):
    ...
    """
    logger.info(f"--- Attempting to reset status for transcript ID: {transcript_id} ---")
    try:
        transcript = db_session.query(db.TranscriptProcessing).filter(db.TranscriptProcessing.id == transcript_id).first()
        if not transcript:
            logger.error(f"No transcript processing entry found with id: {transcript_id}. Reset failed.")
            return

        logger.debug(f"Found transcript to reset: {transcript.id}, current status: {transcript.status}")
        ...
        if video:
            ...
            logger.debug(f"Original video statuses: stage_4_status={video.stage_4_status}, stage_5_status={video.stage_5_status}, stage_6_status={video.stage_6_status}")
            ...
            logger.error(f"Could not find matching video for transcript ID: {transcript_id}. Video status not reset.")
        
        db_session.commit()
        logger.info(f"Successfully reset status for transcript ID: {transcript_id}. It is now ready for reprocessing.")
    except Exception as e:
        logger.error(f"An unexpected error occurred while resetting transcript ID {transcript_id}: {e}", exc_info=True)
        db_session.rollback()
        raise
...
def post_process_transcripts():
    ...
    db_session = db.SessionLocal()
    stop_processing = [False] # Use a mutable list to allow modification within function calls
    ...
    try:
        ...
        transcripts_to_process = db_session.query(db.TranscriptProcessing).filter(
            db.TranscriptProcessing.status.in_(processing_states)
        ...
        for i, tp in enumerate(transcripts_to_process):
            if stop_processing[0]:
                logger.warning("Processing halted due to a critical error (e.g., API quota). Aborting pipeline.")
                break
            
            logger.info(f"--- Processing Transcript ID: {tp.id} (Item {i + 1}/{total_transcripts}) ---")
            ...
            try:
                ...
                logger.info(f"--- Finished processing for Transcript ID: {tp.id}. Final status: '{tp.status}' ---")

            except RuntimeError as e:
                # This catches errors raised by _execute_processing_stage,
                # logs them, and allows the loop to continue to the next transcript.
                logger.error(f"A runtime error occurred while processing transcript {tp.id}. See logs above for details. Moving to the next transcript. Error: {e}")
                # The status is already set by _execute_processing_stage, so we just continue.
                pass
            
    except Exception as e:
        logger.critical(f"A critical unexpected error occurred in the main processing loop: {e}", exc_info=True)
    finally:
...
def run_automated_paragraph_editing():
    ...
    try:
        ...
        for i, sermon in enumerate(sermons_to_process):
            logger.info(f"--- Processing Sermon {i+1}/{total_sermons} (ID: {sermon.id}) for final edit ---")
            logger.debug(f"Sermon {sermon.id} current status: {sermon.status}")

            # 1. Retrieve sermon text
            if not sermon.secondary_cleaning_path:
                logger.error(f"Secondary cleaning path not found for sermon ID: {sermon.id}. Skipping final edit for this sermon.")
                continue
            try:
                logger.debug(f"Reading sermon text from: {sermon.secondary_cleaning_path}")
                with open(sermon.secondary_cleaning_path, 'r') as f:
                    transcript_text = f.read()
                logger.debug(f"Successfully read sermon text for sermon {sermon.id}.")
            except FileNotFoundError:
                logger.error(f"Sermon text file not found at {sermon.secondary_cleaning_path}. Skipping final edit for this sermon.")
                continue
            ...
            if sermon.metadata_path:
                try:
                    ...
                    logger.warning(f"Could not load outline for sermon {sermon.id}: {e}. Proceeding without it.")
                    pass 
            ...
            if not paragraph_file_path:
                ...
                continue
            logger.debug(f"Paragraphs JSON file path for sermon {sermon.id} is: {paragraph_file_path}")
            ...
            if paragraph_file_path.exists():
                try:
                    ...
                    logger.warning(f"Could not load or parse existing paragraphs.json for sermon {sermon.id}: {e}. Rebuilding data.")
                    pass
            
            if not paragraphs_data:
                logger.debug("paragraphs.json data not loaded or file doesn't exist. Building new data structure.")
            ...
            if edited_transcript:
                ...
                try:
                    logger.debug(f"Saving final edited transcript to: {final_transcript_path}")
                    with open(final_transcript_path, 'w') as f:
                        f.write(edited_transcript)
                    logger.info(f"Successfully saved final edited transcript for sermon {sermon.id}.")
                    
                    # 5. Update status
                    logger.debug(f"Updating status to 'final_edit_complete' for sermon {sermon.id}")
                    sermon.status = 'final_edit_complete'
                    session.commit()
                    logger.info(f"Successfully updated status for sermon {sermon.id}.")

                except IOError as e:
                    logger.error(f"Error saving final transcript for sermon {sermon.id}: {e}", exc_info=True)
                    session.rollback()
            else:
                logger.error(f"Final edit process for sermon {sermon.id} did not return an edited transcript. Status not updated.")
        
        logger.info("--- Automated Final Paragraph Editing Finished ---")

    except Exception as e:
        logger.critical(f"A critical unexpected error occurred during automated editing: {e}", exc_info=True)
        session.rollback()
    finally:
...
class EditParagraphs:
    ...
    def select_sermon(self):
        ...
        try:
            ...
            editable_states = ["metadata_generation_complete", "sermon_export_complete", "final_edit_complete"]
            logger.debug(f"Querying for sermons in states: {editable_states}")

            sermons = session.query(db.TranscriptProcessing).filter(
                db.TranscriptProcessing.status.in_(editable_states)
            ).order_by(db.TranscriptProcessing.id).all()

            if not sermons:
                logger.info("No sermons available for editing.")
                print("No sermons are currently in a state ready for editing (metadata complete or later).")
                return []

            print("\n--- Available Sermons for Editing ---")
            for sermon in sermons:
                print(f"  ID: {sermon.id:<5} Status: {sermon.status:<30} Path: ...{str(Path(sermon.raw_transcript_path).name)}")
            print("-" * 35)

            selected_id_str = input("Enter sermon ID to edit, 'all' for all, 'reset <ID>', or 'reset all': ")
            ...
            if selected_id_str.lower().startswith('reset '):
                try:
                    ...
                    logger.debug(f"Calling _reset_sermon_editing_state for ID: {reset_id}")
            ...
            try:
                ...
                if selected_sermon:
                    ...
                    logger.warning(f"No sermon found with ID: {selected_id}. Returning empty list.")
                    print(f"No sermon found with ID: {selected_id}")
                    return []
            except ValueError:
                logger.error(f"Invalid input from user: '{selected_id_str}'. Not a number, 'all', or 'reset'.")
                print("Invalid input. Please enter a number, 'all', or a reset command.")
                return []

        except Exception as e:
            logger.critical(f"An unexpected error occurred in select_sermon: {e}", exc_info=True)
            return []
        finally:
    ...
    def _reset_sermon_editing_state(self, sermon_id):
        ...
        try:
            ...
            if paragraph_file_path and paragraph_file_path.exists():
                ...
                logger.info(f"No paragraphs.json found for sermon ID: {sermon_id}. Nothing to delete.")

            # Reset status in DB
            new_status = "sermon_export_complete"
            logger.debug(f"Updating status of sermon ID {sermon.id} from '{sermon.status}' to '{new_status}'")
    ...
    def _reset_all_sermons_editing_state(self):
        ...
        try:
            ...
            logger.critical(f"An error occurred while resetting all sermons: {e}", exc_info=True)
            if session.is_active:
                session.rollback()
            return "RESET_FAILED"
        finally:
            if session.is_active:
                logger.debug("Closing database session for _reset_all_sermons_editing_state.")
                session.close()
    ...
    def _build_paragraphs_json_data(self, transcript_text, outline):
        ...
        for i, paragraph in enumerate(paragraphs):
            ...
            try:
                ...
                logger.error(f"Error reading prompt file for paragraph {i+1}: {e}", exc_info=True)
                paragraphs_data.append({'index': i, 'original': paragraph, 'prompt': '[PROMPT FILE NOT FOUND]', 'edited': None})
            except Exception as e:
                logger.error(f"Error creating prompt for paragraph {i + 1}: {e}", exc_info=True)
                paragraphs_data.append({'index': i, 'original': paragraph, 'prompt': '[ERROR CREATING PROMPT]', 'edited': None})

        logger.info(f"Finished building paragraph data structure for {total_paragraphs} paragraphs.")
    ...
    def _save_paragraphs_to_file(self, data, file_path):
        ...
        try:
            ...
            logger.info(f"Successfully saved progress for {len(data)} paragraphs to {file_path}")
        except Exception as e:
    ...
    def edit_paragraphs(self, paragraphs_data, paragraph_file_path):
        ...
        if num_to_process == 0:
            logger.info("All paragraphs have already been edited. Reassembling transcript.")
            # Reassemble the full transcript from the loaded data
            edited_transcript = "\n\n".join(p.get('edited', p.get('original', '')) for p in paragraphs_data)
            return edited_transcript

        logger.info(f"Starting sequential processing of {num_to_process} remaining paragraphs out of {total_paragraphs} total.")
        ...
        for i, paragraph_item in enumerate(paragraphs_to_process):
            ...
            logger.info(f"Processing paragraph {index + 1}/{total_paragraphs} ({i + 1}/{num_to_process} in this run).")

            if not prompt or prompt.startswith('[ERROR'):
                logger.warning(f"No valid prompt for paragraph {index}. Using original text.")
            ...
            max_retries = 3
            for attempt in range(max_retries):
                logger.debug(f"Attempt {attempt + 1}/{max_retries} for paragraph {index}.")
                try:
                    ...
                    if attempt > 0:
                        ...
                        logger.debug(f"Successfully processed paragraph {index} on first attempt.")
                    ...
                    logger.critical(f"!!! CRITICAL: Ollama API processing issue. Halting all editing. Error: {e}.", exc_info=True)
                    return None # Stop editing and return early

                except RuntimeError as e:
                    logger.warning(f"Attempt {attempt + 1}/{max_retries} failed for paragraph {index}: {e}")
                    if attempt == max_retries - 1:
                        logger.error(f"All {max_retries} attempts failed for paragraph {index}. The original text will be used.")
        ...
        logger.info(f"Final transcript reassembled. Total length: {len(final_transcript)} characters.")
    ...
    def run_editor(self):
        ...
        if sermons_to_process == "RESET_ACTION":
            ...
            print("Reset action completed. Please run the editor again to select a sermon.")
            return
        ...
        try:
            ...
            for i, sermon in enumerate(sermons_to_process):
                logger.info(f"--- Processing Sermon {i+1}/{total_sermons} (ID: {sermon.id}) ---")
                ...
                if not sermon.secondary_cleaning_path or not Path(sermon.secondary_cleaning_path).exists():
                    ...
                    print(f"ERROR: Source text file not found for sermon {sermon.id}. Cannot edit.")
                    continue
                with open(sermon.secondary_cleaning_path, 'r') as f:
                    transcript_text = f.read()
                ...
                paragraphs_data = None
                if paragraph_file_path and paragraph_file_path.exists():
                    logger.debug(f"Found existing paragraphs file: {paragraph_file_path}")
                    try:
                        with open(paragraph_file_path, 'r') as f:
                            paragraphs_data = json.load(f)
                        logger.info(f"Resuming from existing paragraphs.json for sermon {sermon.id}.")
                    except (json.JSONDecodeError, IOError) as e:
                         logger.warning(f"Could not load or parse {paragraph_file_path}. Rebuilding. Error: {e}")
                
                if not paragraphs_data:
                    logger.info(f"No existing paragraphs.json found for sermon {sermon.id}. Building new data.")
                ...
                if edited_transcript:
                    ...
                    logger.error(f"Editing failed for sermon {sermon.id}. No transcript was returned. Check for critical errors above.")
                    print(f"ERROR: Editing failed for sermon {sermon.id}. The transcript was not saved.")
        
        except Exception as e:
            logger.critical(f"An unexpected error occurred during the editor run: {e}", exc_info=True)
            if session.is_active:
                session.rollback()
        finally:
