...
def process_transcription():
    ...
    try:
        ...
        if process_info:
            ...
            if num_processes == 1:
                logger.warning("An instance of 'controller.py' is already running on the remote machine:")
                logger.warning(process_info)
                logger.warning("Please wait for it to complete or manually stop it before starting a new process.")
                return

            elif num_processes > 1:
                logger.warning("Multiple instances of 'controller.py' are running on the remote machine:")
                pids = []
                for i, process_line in enumerate(processes):
                    ...
                    if len(parts) > 1:
                        pid = parts[1]
                        pids.append(pid)
                        logger.info(f"{i + 1}: PID: {pid}, Details: {' '.join(parts[10:])}")
                ...
                while True:
                    try:
                        ...
                        if 1 <= choice <= num_processes:
                            ...
                            logger.warning(f"Invalid choice. Please enter a number between 1 and {num_processes}.")
                    except ValueError:
        ...
        logger.info("Remote transcription process initiated successfully in the background.")
        logger.info("You can monitor its progress via the 'controller.log' file on the remote machine.")
