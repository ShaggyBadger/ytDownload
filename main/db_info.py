...
def display_db_info():
    ...
    try:
        ...
        stage1_complete = (
            db.query(Video).filter(Video.stage_1_status == "completed").count()
        )
        stage2_pending = (
            db.query(Video).filter(Video.stage_2_status == "pending").count()
        )
        stage2_complete = (
            db.query(Video).filter(Video.stage_2_status == "completed").count()
        )
        stage2_failed = db.query(Video).filter(Video.stage_2_status == "failed").count()
        stage3_pending = (
            db.query(Video).filter(Video.stage_3_status == "pending").count()
        )
        stage3_processing = (
            db.query(Video).filter(Video.stage_3_status == "processing").count()
        )
        stage3_complete = (
            db.query(Video).filter(Video.stage_3_status == "complete").count()
        )
        ...
        tp_raw = (
            db.query(TranscriptProcessing)
            .filter(TranscriptProcessing.status == "raw_transcript_received")
            .count()
        )
        tp_initial_cleaning = (
            db.query(TranscriptProcessing)
            .filter(TranscriptProcessing.status == "initial_cleaning_complete")
            .count()
        )
        tp_secondary_cleaning = (
            db.query(TranscriptProcessing)
            .filter(TranscriptProcessing.status == "secondary_cleaning_complete")
            .count()
        )
        tp_metadata_gen = (
            db.query(TranscriptProcessing)
            .filter(TranscriptProcessing.status == "metadata_generation_complete")
            .count()
        )
        tp_sermon_export = (
            db.query(TranscriptProcessing)
            .filter(TranscriptProcessing.status == "sermon_export_complete")
            .count()
        )
        tp_quota_exceeded = (
            db.query(TranscriptProcessing)
            .filter(TranscriptProcessing.status.like("%_quota_exceeded"))
            .count()
        )
        tp_failed = (
            db.query(TranscriptProcessing)
            .filter(TranscriptProcessing.status.like("%_failed%"))
            .count()
            - tp_quota_exceeded
        )
        ...
        logger.info(
            f"  - Raw Transcript Received (ready for Initial Cleaning): {tp_raw}"
        )
        logger.info(
            f"  - Initial Cleaning Complete (ready for Secondary Cleaning): {tp_initial_cleaning}"
        )
        logger.info(
            f"  - Secondary Cleaning Complete (ready for Metadata Generation): {tp_secondary_cleaning}"
        )
        logger.info(
            f"  - Metadata Generation Complete (ready for Sermon Export): {tp_metadata_gen}"
        )
        ...
        logger.info(
            "Legend: C=Completed, P=Pending, F=Failed, Q=Quota Exceeded, U=Unknown/Not Applicable"
        )
        logger.info("Video Stages: 1=Metadata, 2=MP3 Download, 3=Transcription")
        logger.info(
            "PP Stages: 1=Initial Clean, 2=Secondary Clean, 3=Metadata Gen, 4=Sermon Export"
        )
        logger.info("---")
        # --- Header Row ---
        logger.info(
            f"{'ID':<4} | {'V1':^2} {'V2':^2} {'V3':^2} | {'PP1':^3} {'PP2':^3} {'PP3':^3} {'PP4':^3} | Errors"
        )
        logger.info(
            "-----|----------|-----------------|---------------------------------"
        )
        ...
        all_videos_with_tp = (
            db.query(Video, TranscriptProcessing)
            .outerjoin(TranscriptProcessing, Video.id == TranscriptProcessing.video_id)
            .order_by(Video.id)
            .all()
        )
        ...
        for video, tp in all_videos_with_tp:
            ...
            # Print the line
            logger.info(
                f"{video.id:<4} | {v1_char:^2} {v2_char:^2} {v3_char:^2} | {pp_chars_str[0]:^3} {pp_chars_str[1]:^3} {pp_chars_str[2]:^3} {pp_chars_str[3]:^3} | {error_snippet}"
            )
