...
class Editor:
    ...
    def _build_paragraphs_json_data(self, transcript_text, job_directory):
        ...
        if metadata_path.exists():
            try:
                ...
                logger.info(f"Successfully read speaker tone '{speaker_tone}' from {metadata_path}")
            except Exception:
                logger.error(f"Error reading or parsing {metadata_path} to get speaker tone.", exc_info=True)
        else:
            logger.warning(f"Metadata file not found at {metadata_path}. Using default speaker tone: '{speaker_tone}'.")
        ...
        for i, paragraph in enumerate(paragraphs):
            ...
            try:
                ...
                logger.debug(f"Using prompt template: {prompt_template_path.name} for paragraph {i+1}/{total_paragraphs}")
                ...
                paragraphs_data.append({'index': i, 'original': paragraph, 'prompt': prompt, 'edited': None})

            except FileNotFoundError:
                logger.error(f"Prompt file not found: {prompt_template_path}", exc_info=True)
    ...
    def _load_paragraphs_from_file(self, file_path):
        ...
        try:
            ...
            logger.error(f"An unexpected error occurred loading paragraphs from {file_path}", exc_info=True)
            return None
    ...
    def run_editor(self):
        ...
        with get_session() as session:
            ...
            format_gemini_stage = session.query(JobStage).filter_by(job_id=self.job_id, stage_name="format_gemini").first()
            if not format_gemini_stage or not format_gemini_stage.output_path:
                logger.warning(f"Formatted transcript path not found in 'format_gemini' stage for Job ID: {self.job_id}. Cannot create paragraphs.json.")
                return

            transcript_path = Path(format_gemini_stage.output_path)
            if not transcript_path.exists():
                logger.error(f"Transcript file not found at the path specified in the database: {transcript_path}")
                return

            paragraph_file_path = self._get_paragraph_file_path(job.job_directory)
            if paragraph_file_path.exists():
                logger.info(f"Paragraphs JSON file already exists at {paragraph_file_path}. Skipping creation.")
            else:
                ...
                logger.debug(f"Updating database for 'format_gemini' stage with path: {paragraph_file_path}")
                format_gemini_stage.paragraph_json_path = str(paragraph_file_path)
                session.commit()
                logger.info(f"Committed paragraph_json_path to database for Job ID: {self.job_id}")

    def process_paragraphs_for_editing(self):
        """Loads paragraphs.json and sends unedited ones to the Ollama API."""
        logger.info(f"Starting paragraph-by-paragraph editing for Job ID: {self.job_id}")
        with get_session() as session:
            job = session.query(JobInfo).filter(JobInfo.id == self.job_id).first()
            if not job:
            ...
            if not paragraph_file_path.exists():
                logger.error(f"Paragraphs JSON file not found at {paragraph_file_path}. Cannot perform editing.")
                return
            ...
            for i, p_entry in enumerate(paragraphs_data):
                if p_entry.get('edited') is None:
                    ...
                    with self.console.status(status_message, spinner=config.SPINNER):
                        try:
                            ollama_response = self.ollama_client.submit_prompt(p_entry['prompt'])
                            logger.debug(f"Ollama response for paragraph {i+1}: '{ollama_response[:100]}...'")
                            p_entry['edited'] = ollama_response
                            edited_this_run += 1
                            self._save_paragraphs_to_file(paragraphs_data, paragraph_file_path) # Save after each successful edit
                        except Exception as e:
                            logger.error(f"Error processing paragraph {i+1} for Job ID {self.job_id} with Ollama.", exc_info=True)
                            p_entry['edited'] = f"[ERROR] - See logs for details."
                            self._save_paragraphs_to_file(paragraphs_data, paragraph_file_path) # Save error state
                else:
                    logger.debug(f"Paragraph {i+1}/{total_paragraphs} already edited. Skipping.")
            
            if edited_this_run == 0:
                logger.info(f"No new paragraphs required editing for Job ID {self.job_id}.")
            else:
                logger.info(f"Finished editing {edited_this_run} paragraphs for Job ID {self.job_id}.")

            if all(p.get('edited') is not None for p in paragraphs_data):
                logger.info(f"All {total_paragraphs} paragraphs for Job ID {self.job_id} are now edited.")
                edit_llm_stage = session.query(JobStage).filter_by(job_id=self.job_id, stage_name="edit_local_llm").first()
                if edit_llm_stage and edit_llm_stage.state != StageState.success:
                    ...
                    logger.info(f"Job ID {self.job_id}: 'edit_local_llm' stage marked as SUCCESS in the database.")
            else:
                logger.warning(f"Job ID {self.job_id}: Not all paragraphs are edited. 'edit_local_llm' stage remains pending.")
    ...
    def build_paragraph_editing_score(self, original_paragraph_dict, edited_paragraph_dict):
        ...
        logger.warning("build_paragraph_editing_score is a placeholder and not yet implemented.")
        pass
