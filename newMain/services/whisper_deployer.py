...
class Deployer:
    def __init__(self):
        ...
        logger.debug(f"Deployer service initialized. FastAPI URL: {self.fastapi_base_url}")
        ...
        with self.console.status("Searching for jobs to deploy...", spinner=config.SPINNER) as status:
            try:
                with get_session() as session:
                    ...
                    logger.info(f"Found {len(jobs_to_deploy)} job(s) ready for deployment.")
                    for job in jobs_to_deploy:
                        try:
                            self._deploy_job(session, job)
                        except Exception as e:
                            logger.error(f"Error deploying individual job {job.job_ulid}: {e}", exc_info=True)
                            self.console.print(f"[red]Error deploying job {job.job_ulid}. Check logs.[/red]")
            except Exception as e:
                logger.critical(f"Critical error during deployment of pending jobs: {e}", exc_info=True)
                self.console.print(f"[bold red]Critical error during deployment. Check logs for details.[/bold red]")


    def _find_jobs_to_deploy(self, session: Session):
        logger.debug("Querying for jobs ready for deployment (extract_audio success, transcribe_whisper pending/failed).")
        jobs = (
    ...
    def _deploy_job(self, session: Session, job: JobInfo):
        logger.info(f"Deploying job {job.id} (ULID: {job.job_ulid}) to transcription server.")
        with self.console.status(f"Deploying job [bold cyan]{job.job_ulid}[/bold cyan]...", spinner=config.SPINNER) as status:
            ...
            if not whisper_stage:
                status.stop()
                logger.error(f"Error: Could not find whisper stage for job {job.job_ulid}. This indicates a pipeline configuration issue.")
            ...
            if not file_path.exists():
                status.stop()
                logger.error(f"Error: Audio file not found at {file_path} for job {job.job_ulid}. Updating stage to FAILED.")
            ...
            try:
                ...
                with open(file_path, 'rb') as f:
                    ...
                    logger.debug(f"Sending POST request to {url} with ULID: {job.job_ulid}.")
                    ...
                    if response_data.get("status") == "deployed":
                        ...
                        session.commit()
                        logger.info(f"Job {job.job_ulid} successfully deployed and stage updated to RUNNING.")
                    else:
                        status.stop()
                        logger.error(f"Server returned non-deployed status for job {job.job_ulid}: {response_data.get('status')}. Marking stage as FAILED.")
                ...
                logger.error(f"Network error deploying job {job.job_ulid}: {error_message}. Marking stage as FAILED.", exc_info=True)
                ...
                status.stop()
                logger.critical(f"An unexpected error occurred deploying job {job.job_ulid}: {e}. Marking stage as FAILED.", exc_info=True)
        ...
        with self.console.status("Checking for completed transcription jobs...", spinner=config.SPINNER) as status:
            try:
                with get_session() as session:
                    ...
                    if not running_jobs:
                        status.update("No jobs are currently processing on the server.", spinner=config.SPINNER)
                        self.console.print("No jobs are currently processing on the server.", style="green")
                        logger.info("No jobs found currently running for transcription.")
                        return
                    
                    status.update(f"Found {len(running_jobs)} job(s) being transcribed. Checking status...", spinner=config.SPINNER)
                    logger.info(f"Found {len(running_jobs)} jobs with 'transcribe_whisper' stage RUNNING.")
                    for job in running_jobs:
                        try:
                            self._check_and_retrieve_job(session, job)
                        except Exception as e:
                            logger.error(f"Error checking/retrieving job {job.job_ulid}: {e}", exc_info=True)
                            self.console.print(f"[red]Error processing job {job.job_ulid}. Check logs.[/red]")
            except Exception as e:
                logger.critical(f"Critical error during check for completed jobs: {e}", exc_info=True)
                self.console.print(f"[bold red]Critical error during status check. Check logs for details.[/bold red]")
        ...
        with self.console.status(f"Attempting to manually recover job [bold cyan]{job_ulid}[/bold cyan]...", spinner=config.SPINNER) as status:
            try:
                with get_session() as session:
                    ...
                    if not job:
                        status.update(f"Job [bold red]{job_ulid}[/bold red] not found in the local database.", spinner=config.SPINNER)
                        self.console.print(f"Job [bold red]{job_ulid}[/bold red] not found in the local database.", style="red")
                        logger.warning(f"Job ULID {job_ulid} not found in the local database for manual recovery.")
                        return
                    
                    status.update(f"Found job [bold cyan]{job.job_ulid}[/bold cyan]. Checking server status...", spinner=config.SPINNER)
                    logger.info(f"Job {job.job_ulid} found in local DB. Proceeding to check server status.")
                    self._check_and_retrieve_job(session, job)
            except Exception as e:
                logger.critical(f"Critical error during manual recovery for Job ULID {job_ulid}: {e}", exc_info=True)
                self.console.print(f"[bold red]Critical error during manual recovery. Check logs for details.[/bold red]")
    ...
    def _check_and_retrieve_job(self, session: Session, job: JobInfo):
        logger.info(f"Checking server status and potentially retrieving for Job ULID: {job.job_ulid}.")
        with self.console.status(f"Checking status for job [bold cyan]{job.job_ulid}[/bold cyan]...", spinner=config.SPINNER) as status:
            whisper_stage = next((s for s in job.stages if s.stage_name == "transcribe_whisper"), None)
            if not whisper_stage: 
                status.stop()
                logger.error(f"Error: No transcribe_whisper stage found for job {job.job_ulid}. This indicates a pipeline issue.")
            ...
            try:
                status.update(f"Requesting server status for job [bold cyan]{job.job_ulid}[/bold cyan] from {status_url}...")
                logger.debug(f"Requesting server status for job {job.job_ulid} from {status_url}.")
                status_response = requests.get(status_url, timeout=10)
                status_response.raise_for_status()
                server_job_status = status_response.json().get("status")
                logger.debug(f"Server reported status for job {job.job_ulid}: {server_job_status}")
                ...
                if server_job_status == "completed":
                    ...
                    logger.info(f"Server reports job {job.job_ulid} as COMPLETED. Initiating transcript retrieval.")
                    self._retrieve_transcript(session, job, whisper_stage)
                    status.stop()
                elif server_job_status == "failed":
                    status.stop()
                    logger.error(f"Server reports job {job.job_ulid} as FAILED. Updating stage to FAILED.")
                    ...
                    logger.info(f"Job {job.job_ulid} is still PROCESSING on server. Status: {server_job_status}.")

            except requests.exceptions.RequestException as e:
                status.stop()
                error_message = str(e.response.text if e.response else e)
                logger.error(f"Network error checking status for job {job.job_ulid}: {error_message}. Marking stage as FAILED.", exc_info=True)
                ...
                status.stop()
                logger.critical(f"An unexpected error occurred while checking status for job {job.job_ulid}: {e}. Marking stage as FAILED.", exc_info=True)
    ...
    def _retrieve_transcript(self, session: Session, job: JobInfo, whisper_stage: JobStage):
        ...
        with self.console.status(f"Retrieving transcript for job [bold cyan]{job.job_ulid}[/bold cyan]...", spinner=config.SPINNER) as status:
            ...
            try:
                status.update(f"Downloading transcript from server for job [bold cyan]{job.job_ulid}[/bold cyan] from {retrieve_url}...")
                logger.debug(f"Downloading transcript for job {job.job_ulid} from {retrieve_url}.")
                ...
                logger.debug(f"Saving transcript content (length: {len(transcript_content)}) to {transcript_path}.")
                ...
                logger.info(f"Successfully saved transcript for job {job.job_ulid} to {transcript_path}.")
                
                whisper_stage.state = StageState.success
                whisper_stage.output_path = str(transcript_path)
                session.commit()
                logger.info(f"Job {job.job_ulid}: 'transcribe_whisper' stage marked as SUCCESS with output path: {transcript_path}.")

            except requests.exceptions.RequestException as e:
                logger.error(f"Network error retrieving transcript for job {job.job_ulid}: {e}. Marking stage as FAILED.", exc_info=True)
                ...
                logger.critical(f"An unexpected error occurred while retrieving transcript for job {job.job_ulid}: {e}. Marking stage as FAILED.", exc_info=True)
