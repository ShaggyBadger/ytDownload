        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                if not gemini_result.ok:
                    logger.error(f"Gemini call failed (Attempt {attempt + 1}). Error: {gemini_result.error_message}")
                    if "quota" in str(gemini_result.error_message).lower():
                        self.console.print("[bold red]Gemini quota error. Aborting.[/bold red]")
                        return None
                    if attempt < max_retries - 1:
                        time.sleep(2)
                        continue
                    else:
                        self.console.print("[bold red]Gemini call failed after multiple retries. Aborting.[/bold red]")
                        return None

                # Gemini call was successful, now check word loss
                current_formatted_text = gemini_result.output
                current_formatted_text = re.sub(r'\n+', '\n', current_formatted_text).strip()
                ...
                logger.debug(f"Attempt {attempt + 1}: Formatted words: {formatted_word_count}, Word loss: {word_loss_percentage:.2f}%")

                if word_loss_percentage <= word_loss_threshold:
                    logger.info(f"Word loss ({word_loss_percentage:.2f}%) is within tolerance. Success.")
                    formatted_text = current_formatted_text
                    break # Success
                else:
                    logger.warning(f"Word loss ({word_loss_percentage:.2f}%) exceeds threshold of {word_loss_threshold}%. Retrying...")
                    if attempt < max_retries - 1:
                        time.sleep(2)
                    else:
                        logger.error("Exceeded max retries due to high word loss.")
                        self.console.print("[bold red]Failed to format due to high word loss after multiple retries.[/bold red]")
                        return None
