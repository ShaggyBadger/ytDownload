...
class ChapterBuilder:
    ...
    def build_chapter_document(self):
        ...
        with get_session() as session:
            ...
            if not job:
                logger.error(f"Job with ID {self.job_id} not found in the database. Aborting chapter build.")
            ...
            if metadata_path.exists():
                try:
                    ...
                    logger.error(f"Error loading or parsing metadata from {metadata_path}.", exc_info=True)
                    self.console.print(f"[red]Error loading metadata from {metadata_path}. Check logs.[/red]")
                    return
            else:
                logger.error(f"Metadata file not found for Job ID {job.job_ulid} at {metadata_path}. Aborting chapter build.")
                self.console.print(f"[red]Metadata file not found for job {job.job_ulid}.[/red]")
                return
            ...
            logger.debug(f"Extracted from metadata: Title='{title}', Thesis='{thesis[:50]}...', Summary='{summary[:50]}...'")

            # --- Load Edited Paragraphs ---
            paragraph_json_path = job_directory / config.PARAGRAPHS_FILE_NAME
            paragraphs_data = []
            logger.debug(f"Attempting to load edited paragraphs from {paragraph_json_path}")
            if paragraph_json_path.exists():
                try:
                    with open(paragraph_json_path, 'r') as f:
                        paragraphs_data = json.load(f)
                    logger.info(f"Successfully loaded {len(paragraphs_data)} paragraphs from {paragraph_json_path}.")
                except Exception:
                    logger.error(f"Error loading or parsing paragraphs from {paragraph_json_path}.", exc_info=True)
                    self.console.print(f"[red]Error loading paragraphs from {paragraph_json_path}. Check logs.[/red]")
                    return
            else:
                logger.error(f"Paragraphs JSON file not found for Job ID {job.job_ulid} at {paragraph_json_path}. Aborting chapter build.")
                self.console.print(f"[red]Paragraphs JSON file not found for job {job.job_ulid}.[/red]")
                return
            
            # Ensure all paragraphs are edited before proceeding
            if any(p.get('edited') is None for p in paragraphs_data):
                logger.error(f"Not all paragraphs for Job ID {job.job_ulid} are edited. Aborting chapter build.")
            ...
            if job.video and job.video.upload_date:
                try:
                    ...
                    logger.debug(f"Formatted upload date '{job.video.upload_date}' to '{formatted_date}'.")
                except ValueError:
                    logger.warning(f"Could not parse upload date '{job.video.upload_date}' for Job ID {self.job_id}. Using raw date string.", exc_info=True)
            ...
            logger.info(f"Attempting to save final chapter document to {final_document_path}")
            try:
                with open(final_document_path, 'w') as f:
                    f.write(final_document_content)
                logger.info(f"Successfully built and saved chapter document at {final_document_path}.")
                self.console.print(f"[green]Successfully built chapter document at {final_document_path}[/green]")
            except Exception:
                logger.error(f"Error saving final document to {final_document_path}.", exc_info=True)
                self.console.print(f"[red]Error saving final document to {final_document_path}. Check logs.[/red]")
                return

            # --- Update Database Stage ---
            build_chapter_stage = session.query(JobStage).filter_by(job_id=self.job_id, stage_name="build_chapter").first()
            ...
            if build_chapter_stage:
                ...
                session.commit()
                logger.info(f"Job ID {self.job_id}: 'build_chapter' stage marked as SUCCESS with output path: {final_document_path}.")
                self.console.print(f"[green]Job {job.job_ulid}: 'build_chapter' stage marked as SUCCESS.[/green]")
            else:
                logger.warning(f"Job ID {job.job_ulid}: 'build_chapter' stage not found in database. Cannot update its state.")
                self.console.print(f"[yellow]Job {job.job_ulid}: 'build_chapter' stage not found in database.[/red]")
