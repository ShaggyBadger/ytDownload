...
class Downloader:
    ...
    def run_all(self):
        ...
        try:
            ...
            logger.critical("An unexpected error occurred during run_all job processing loop.", exc_info=True)

    def run_one(self):
        logger.info("Starting run_one to allow user to select a single job for download.")
        try:
            with get_session() as session:
                query = session.query(JobInfo).join(JobStage).filter(
                    JobStage.stage_name == 'download_video',
                    or_(JobStage.state == StageState.pending, JobStage.state == StageState.failed)
                )
                available_jobs = query.all()

                if not available_jobs:
                    self.console.print("No jobs available for download.", style="yellow")
                    logger.info("User initiated run_one, but no jobs were available for download.")
                    return
                ...
                while True:
                    ...
                    if selected_job:
                        ...
                        self.console.print(f"Selected job [green]{selected_job.id}[/green]: {selected_job.video.title}", style="bold green")
            ...
            logger.critical("A critical error occurred during the run_one job selection process.", exc_info=True)
    ...
    def _build_available_jobs(self) -> list:
        ...
        with get_session() as session:
            pending_jobs = session.query(JobInfo).join(JobStage).filter(
                JobStage.stage_name == 'download_video',
                or_(JobStage.state == StageState.pending, JobStage.state == StageState.failed)
            ).all()
    ...
    def _download_job(self, job_package: dict):
        ...
        try:
            with get_session() as session:
                ...
                if job_stage:
                    logger.debug(f"Updating Job ID {job_id}, Stage 'download_video' to RUNNING.")
            ...
            def progress_hook(d):
                if d.get('status') == 'downloading':
                    logger.debug(f"yt-dlp progress: {d['_percent_str']} of {d['_total_bytes_str']} at {d['_speed_str']}")
                if d.get('status') == 'finished':
                    logger.info("yt-dlp download finished.")

            def postprocessor_hook(d):
                if d.get('status') == 'started':
                    logger.info(f"yt-dlp post-processing started: {d.get('postprocessor')}")
                if d.get('status') == 'finished':
            ...
            with self.console.status(f"[bold green]Downloading {job_package.get('video_url')}...", spinner=config.SPINNER):
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ydl.download([job_package.get('video_url')])

            with self.console.status("[bold green]Trimming audio...", spinner=config.SPINNER):
                logger.info(f"Loading {full_audio_path} for trimming.")
                audio = AudioSegment.from_file(full_audio_path)
                start_ms = job_package.get('audio_start_time') * 1000
                end_ms = job_package.get('audio_end_time') * 1000 if job_package.get('audio_end_time') > 0 else len(audio)
                logger.info(f"Trimming audio from {start_ms}ms to {end_ms}ms.")
                trimmed_audio = audio[start_ms:end_ms]
                trimmed_audio.export(trimmed_audio_path, format="mp3")
                logger.info(f"Successfully exported trimmed audio to {trimmed_audio_path}")
            ...
            with get_session() as session:
                ...
                if dl_stage:
                    ...
                    logger.info(f"Set Job ID {job_id} Stage 'download_video' to SUCCESS.")
                
                if ea_stage:
                    ea_stage.state = StageState.success
                    ea_stage.finished_at = utcnow()
                    ea_stage.output_path = str(trimmed_audio_path)
                    logger.info(f"Set Job ID {job_id} Stage 'extract_audio' to SUCCESS with output path: {trimmed_audio_path}")
                
                session.commit()
            self.console.print(f"Successfully processed job {job_id}.", style="green")
        except Exception:
            logger.error(f"A critical error occurred during the download/trim process for Job ID {job_id}.", exc_info=True)
            with get_session() as session:
                job_stage = session.query(JobStage).filter_by(job_id=job_id, stage_name='download_video').first()
                if job_stage:
                    ...
                    session.commit()
                    logger.info(f"Set Job ID {job_id} Stage 'download_video' to FAILED in database.")
            self.console.print(f"Error processing job {job_id}. Check logs for details.", style="red")
    ...
    def _secure_delete_file(self, file_path: Path):
        ...
        try:
            ...
            if result.stdout: logger.debug(f"Shred stdout: {result.stdout.strip()}")
            if result.stderr: logger.warning(f"Shred stderr: {result.stderr.strip()}")
            ...
            try:
                ...
                logger.error(f"Error during fallback deletion of {file_path} with os.remove().", exc_info=True)
        except subprocess.CalledProcessError as e:
            logger.error(f"Shred command failed for {file_path}.", exc_info=True)
            if e.stdout: logger.error(f"Shred stdout: {e.stdout.strip()}")
            if e.stderr: logger.error(f"Shred stderr: {e.stderr.strip()}")
        except Exception:
            logger.critical(f"An unexpected critical error occurred during deletion of {file_path}.", exc_info=True)
