...
class IngestLink:
    def __init__(self, data_packet):
        ...
        logger.debug(f"IngestLink initialized for link: {self.link}, start: {self.start_seconds}s, end: {self.end_seconds}s")
    ...
    def _get_video_metadata(self, url):
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                logger.debug(f"Successfully fetched metadata for yt_id: {video_details.get('yt_id')}")
                return video_details
        except yt_dlp.utils.DownloadError:
            logger.error(f"yt-dlp failed to fetch metadata for URL: {url}", exc_info=True)
            return {}
        except Exception:
            logger.critical(f"An unexpected error occurred during yt-dlp metadata extraction for {url}", exc_info=True)
            return {}
    ...
    def ingest_into_db(self):
        ...
        if not yt_id:
            logger.error(f"Could not ingest link because YouTube ID extraction failed for: {self.link}")
            return None
        ...
        try:
            with get_session() as session:
                ...
                if not video_info:
                    logger.info(f"No existing VideoInfo found for yt_id: {yt_id}. Fetching new metadata.")
                    metadata = self._get_video_metadata(self.link)
                    
                    if not metadata:
                        logger.error(f"Failed to fetch metadata for {self.link}. Aborting ingestion.")
                        return None
                    
                    video_info = VideoInfo(**metadata)
                    session.add(video_info)
                    session.flush() # Ensure video_info gets an ID
                    logger.info(f"New VideoInfo created with ID: {video_info.id} for yt_id: {yt_id}")
                else:
                    logger.info(f"Found existing VideoInfo with ID: {video_info.id} for yt_id: {yt_id}")
                ...
                logger.info(f"New JobInfo created with temporary ID: {new_job.id} and ULID: {new_job.job_ulid}")
                
                new_job.job_directory = build_job_directory_path(new_job.job_ulid, new_job.id)
                logger.info(f"Set job directory for Job ID {new_job.id} to: {new_job.job_directory}")

                # Create JobStage entries for the new job
                logger.debug(f"Creating {len(STAGE_ORDER)} JobStage entries for Job ID {new_job.id}")
                ...
                session.commit()
                logger.info(f"Successfully committed Job ID {new_job.id} and its {len(STAGE_ORDER)} stages to the database.")
                self.console.print(f"[bold green]Successfully ingested job for {self.link} (Job ID: {new_job.id})[/bold green]")
                return new_job
        except Exception:
            logger.critical("A critical error occurred during the database transaction.", exc_info=True)
            self.console.print("[bold red]A critical error occurred during ingestion. Check the logs for details.[/bold red]")
            return None
...
class ManualJobSetup:
    ...
    def run(self):
        ...
        while solicit_input:
            ...
            logger.debug(f"User input received: link='{link}', start_time='{start_time_str}', end_time='{end_time_str}'")

            start_seconds = self._parse_time_to_seconds(start_time_str)
            end_seconds = self._parse_time_to_seconds(end_time_str)

            if start_seconds is None:
                self.console.print(f"[red]Error: Invalid start time format: '{start_time_str}'. Please use HH:MM:SS, MM:SS, or SS.[/red]")
                logger.warning(f"Invalid start time format provided by user: '{start_time_str}'")
                continue

            if end_seconds is None:
                self.console.print(f"[red]Error: Invalid end time format: '{end_time_str}'. Please use HH:MM:SS, MM:SS, or SS.[/red]")
                logger.warning(f"Invalid end time format provided by user: '{end_time_str}'")
                continue

            if end_seconds > 0 and end_seconds < start_seconds: # end_seconds=0 means full clip
                self.console.print("[red]Error: End time cannot be before start time. Please re-enter.[/red]")
                logger.warning(f"User provided end time ({end_seconds}) before start time ({start_seconds}).")
                continue
            
            data_packet = {'link': link, 'start_time': start_seconds, 'end_time': end_seconds}
    ...
    def _parse_time_to_seconds(self, time_str):
        ...
        try:
            ...
            if len(parts) == 3:  # HH:MM:SS
                ...
                logger.warning(f"Invalid time format: '{time_str}' has incorrect number of parts.")
                return None
            logger.debug(f"Parsed '{time_str}' to {seconds} seconds.")
            return seconds
        except ValueError:
            logger.error(f"Failed to parse time string to integer: '{time_str}'", exc_info=True)
            return None
