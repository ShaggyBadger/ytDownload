...
class MetadataExtractor:
    ...
    def _load_metadata_json(self, metadata_path: Path) -> Dict[str, Any]:
        ...
        if metadata_path.exists():
            try:
                ...
                logger.error(
                    f"Error decoding JSON from {metadata_path}.", exc_info=True
                )
                self.console.print(
                    f"[red]Error decoding JSON from {metadata_path}. Check logs.[/red]"
                )
            except Exception:
                logger.error(
                    f"Error loading metadata from {metadata_path}.", exc_info=True
                )
                self.console.print(
            ...
            logger.debug(
                f"Metadata file not found at {metadata_path}. Returning empty dict."
            )
        return {}
    ...
    def _save_metadata_json(self, metadata: Dict[str, Any], metadata_path: Path):
        ...
        try:
            ...
            self.console.print(
                f"[red]Error saving metadata to {metadata_path}. Check logs.[/red]"
            )
    ...
    def _get_transcript_text(self, session, job_id: int) -> str:
        ...
        if (
            not formatted_transcript_stage
            or formatted_transcript_stage.state != StageState.success
            or not formatted_transcript_stage.output_path
        ):
            logger.error(
                f"Formatted transcript not found or not successful for Job ID {job_id}. Stage output_path: {formatted_transcript_stage.output_path if formatted_transcript_stage else 'N/A'}"
            )
        ...
        if not formatted_transcript_path.is_file():
            logger.error(
                f"Formatted transcript file not found at {formatted_transcript_path} for Job ID {job_id}."
            )
            raise FileNotFoundError(
                f"Formatted transcript file not found at {formatted_transcript_path} for Job ID {job_id}"
            )
            ...
            with get_session() as session:
                ...
                for category in config.METADATA_CATEGORIES:
                    if metadata.get(category) is None:
                        ...
                        if generation_method:
                            ...
                            with self.console.status(
                                ...
                                try:
                                    ...
                                    all_categories_filled = (
                                        False  # Still not fully filled due to error
                                    )
                        else:
                            logger.error(
                            ...
                            self.console.print(
                                f"[red]  Error: Generation method for {category} not found.[/red]"
                            )
                ...
                if all_categories_filled:
                    ...
                    if metadata_stage and metadata_stage.state != StageState.success:
                        ...
                        logger.debug(
                            f"Job ID {job.job_ulid}: 'extract_metadata' stage was already successful or not found."
                        )
                else:
                    logger.warning(
                        f"Job ID {job.job_ulid}: Not all metadata categories filled. 'extract_metadata' stage remains pending/failed."
                    )
                    self.console.print(
                        f"[yellow]Job {job.job_ulid}: Not all metadata categories filled. 'extract_metadata' stage remains pending/failed.[/yellow]"
                    )
        except Exception:
            logger.critical(
