...
class Menu:
        ...
        while True:
            ...
            if action and action["func"]:
                ...
                try:
                    ...
                    logger.critical(
                        f"An unhandled error occurred while running '{action.get('desc')}' in Whisper Deploy Menu.",
                        exc_info=True,
                    )
                    self.console.print(
                        f"[bold red]An unexpected error occurred while running '{action.get('desc')}'. Check logs for details.[/bold red]"
                    )
    ...
    def _deploy(self):
        ...
        try:
            ...
            self.console.print(
                "[red]An error occurred during deployment. Check logs for details.[/red]"
            )
    ...
    def _recover(self):
        ...
        try:
            ...
            self.console.print(
                "[red]An error occurred during job status check. Check logs for details.[/red]"
            )
    ...
    def _manually_recover(self):
        ...
        if job_id:
            try:
                ...
                logger.error(
                    f"Error during manual recovery for Job ID: {job_id}.", exc_info=True
                )
                self.console.print(
                    f"[red]An error occurred during manual recovery for Job ID {job_id}. Check logs for details.[/red]"
                )
        else:
            logger.warning("No Job ID provided for manual recovery.")
            self.console.print(
                "[yellow]No Job ID provided for manual recovery.[/yellow]"
            )
