...
class ChapterBuilderMenu:
    def __init__(self):
        ...
        self.options = {
            '1': {'desc': 'Build chapter for single job (if missing)', 'func': self._select_and_build_single_chapter},
            '2': {'desc': 'Build chapter for all eligible jobs (if missing)', 'func': self._build_all_chapters},
            '3': {'desc': 'Overwrite chapter document by ULID', 'func': self._overwrite_chapter_document},
    ...
    def _get_eligible_jobs_for_chapter_build(self):
        ...
        try:
            with get_session() as session:
                ...
                successful_edit_llm_jobs = session.query(JobStage.job_id).filter(
                ...
                logger.debug(f"Found {len(candidate_jobs_query)} candidate jobs from DB query.")
                ...
                for job_data in candidate_jobs_query:
                    ...
                    logger.debug(f"Job ULID {job_data.job_ulid}: Final document exists on disk: {final_document_exists}")
            ...
            logger.info(f"Identified {len(jobs_list)} eligible jobs for chapter building.")
        except Exception:
            logger.error("Error querying for eligible jobs for chapter build.", exc_info=True)
        return jobs_list
    ...
    def _select_and_build_single_chapter(self):
        ...
        while True:
            ...
            if choice == 'b':
                logger.info("User opted to go back from single chapter build selection.")
                return
            ...
            if selected_job_data:
                ...
                logger.info(f"User selected Job ULID: {selected_job_data['job_ulid']} (ID: {selected_job_data['id']}) for single chapter build.")
                try:
                    chapter_builder_service = ChapterBuilder(job_id=selected_job_data['id'])
                    chapter_builder_service.build_chapter_document()
                    logger.info(f"Successfully built chapter document for Job ULID: {selected_job_data['job_ulid']}.")
                except Exception:
                    logger.error(f"Error building chapter document for Job ULID: {selected_job_data['job_ulid']}.", exc_info=True)
                self.console.input("Press Enter to continue...")
                return
            else:
                logger.warning("Invalid selection for single chapter build: '%s'", choice)
    ...
    def _build_all_chapters(self):
        ...
        logger.info(f"Found {len(jobs_to_build)} eligible jobs for bulk chapter building.")
        for i, job_data in enumerate(jobs_to_build):
            self.console.print(f"[bold white]Processing Job ({i+1}/{len(jobs_to_build)}):[/bold white] {job_data['job_ulid']} - {job_data['title']}")
            logger.info(f"Processing Job ({i+1}/{len(jobs_to_build)}): Job ULID: {job_data['job_ulid']} (ID: {job_data['id']})")
            try:
                chapter_builder_service = ChapterBuilder(job_id=job_data['id'])
                chapter_builder_service.build_chapter_document()
                logger.info(f"Successfully built chapter document for Job ULID: {job_data['job_ulid']}.")
            except Exception:
                logger.error(f"Error building chapter document for Job ULID: {job_data['job_ulid']}.", exc_info=True)
            self.console.print()
    ...
    def _overwrite_chapter_document(self):
        ...
        ulid_input = Prompt.ask("[bold yellow]Enter the Job ULID to overwrite (or 'b' to go back)[/bold yellow]").strip()
        ...
        try:
            with get_session() as session:
                ...
                if not job_info:
                    logger.warning(f"Job with ULID '{ulid_input}' not found for overwrite.")
                    self.console.print(f"[red]Job with ULID '{ulid_input}' not found.[/red]")
                    self.console.input("Press Enter to continue...")
                    return

                metadata_stage = session.query(JobStage).filter_by(job_id=job_info.id, stage_name="extract_metadata").first()
                edit_llm_stage = session.query(JobStage).filter_by(job_id=job_info.id, stage_name="edit_local_llm").first()
                
                logger.debug(f"Job {ulid_input}: Metadata stage state: {metadata_stage.state if metadata_stage else 'N/A'}")
                logger.debug(f"Job {ulid_input}: Edit LLM stage state: {edit_llm_stage.state if edit_llm_stage else 'N/A'}")

                if not metadata_stage or metadata_stage.state != StageState.success:
                    logger.warning(f"Job {ulid_input}: Metadata extraction stage is not successful. Cannot overwrite.")
                    self.console.print(f"[red]Job {ulid_input}: Metadata extraction stage is not successful. Cannot overwrite.[/red]")
                    self.console.input("Press Enter to continue...")
                    return
                
                if not edit_llm_stage or edit_llm_stage.state != StageState.success:
                    logger.warning(f"Job {ulid_input}: Local LLM editing stage is not successful. Cannot overwrite.")
                    self.console.print(f"[red]Job {ulid_input}: Local LLM editing stage is not successful. Cannot overwrite.[/red]")
                    self.console.input("Press Enter to continue...")
                    return

                self.console.print(f"[green]Job {ulid_input} is eligible for chapter overwrite.[/green]")
                logger.info(f"Job {ulid_input} found eligible for overwrite.")
                confirm = Prompt.ask("[bold yellow]Confirm overwrite for this job? (y/n):[/bold yellow]").lower()
                ...
                if confirm == 'y':
                    try:
                        ...
                        chapter_builder_service.build_chapter_document()
                        self.console.print(f"[green]Chapter document for {ulid_input} overwritten.[/green]")
                        logger.info(f"Chapter document for {ulid_input} successfully overwritten.")
                    except Exception:
                        logger.error(f"Error overwriting chapter document for Job ULID: {ulid_input}.", exc_info=True)
                        self.console.print(f"[red]Error overwriting chapter document for {ulid_input}. Check logs.[/red]")
                else:
                    self.console.print("[yellow]Overwrite cancelled.[/yellow]")
                    logger.info(f"Overwrite for Job ULID {ulid_input} cancelled by user.")
                
                self.console.input("Press Enter to continue...")
        except Exception:
            logger.error(f"An error occurred during the overwrite chapter document process for ULID '{ulid_input}'.", exc_info=True)
    ...
    def run(self):
        ...
        while True:
            ...
            logger.info("Executing Chapter Builder Menu action: '%s'", selected_option.get('desc'))
            try:
                action_func()
            except Exception:
                logger.critical(f"An unhandled error occurred while running '{selected_option.get('desc')}' in Chapter Builder Menu.", exc_info=True)
