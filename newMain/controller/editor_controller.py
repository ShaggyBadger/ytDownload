...
class EditorMenu:
    def __init__(self):
        ...
        self.options = {
            ...
            '3': {'desc': 'Process Edited Paragraphs', 'func': self._run_paragraph_editing_menu},
    ...
    def _select_and_build_paragraph_json_for_single_job(self):
        ...
        if selected_job_data:
            ...
            logger.info(f"User selected Job ULID: {selected_job_data['job_ulid']} (ID: {selected_job_data['id']}) for single paragraph JSON build.")
            try:
                editor_service = Editor(job_id=selected_job_data['id'])
                editor_service.run_editor() # This method builds the JSON
                logger.info(f"Successfully ran editor for Job ULID: {selected_job_data['job_ulid']}.")
            except Exception:
                logger.error(f"Error building paragraph JSON for Job ULID: {selected_job_data['job_ulid']}.", exc_info=True)
        else:
    ...
    def _get_selected_sermon_for_json_build(self):
        ...
        while True:
            ...
            if choice == 'b':
                logger.info("User opted to go back from paragraph JSON build selection.")
                return None
            
            selected_job_data = job_map.get(choice)
            if selected_job_data:
                logger.debug(f"Returning selected job data for ULID: {selected_job_data['job_ulid']}")
                return selected_job_data
            else:
                logger.warning("Invalid selection for paragraph JSON build: '%s'", choice)
    ...
    def _get_eligible_jobs_for_json_build(self):
        ...
        try:
            with get_session() as session:
                ...
                logger.debug(f"Found {len(jobs_query_results)} candidate jobs from DB query.")
            ...
            logger.info(f"Identified {len(jobs_list)} eligible jobs for paragraph JSON build.")
        except Exception:
            logger.error("Error querying for eligible jobs for paragraph JSON build.", exc_info=True)
        return jobs_list
    ...
    def _build_all_paragraph_jsons(self):
        ...
        logger.info(f"Found {len(eligible_jobs)} eligible jobs for bulk paragraph JSON build.")
        for i, job_data in enumerate(eligible_jobs):
            ...
            logger.info(f"Processing Job ({i+1}/{len(eligible_jobs)}): Job ULID: {job_data['job_ulid']} (ID: {job_data['id']})")
            try:
                editor_service = Editor(job_id=job_data['id'])
                editor_service.run_editor()
                logger.info(f"Successfully ran editor for Job ULID: {job_data['job_ulid']}.")
            except Exception:
                logger.error(f"Error building paragraph JSON for Job ULID: {job_data['job_ulid']}.", exc_info=True)
            self.console.print()
    ...
    def _run_paragraph_editing_menu(self):
        ...
        while True:
            ...
            logger.debug("User selected menu option in paragraph editing sub-menu: '%s'", choice)
            ...
            logger.info("Executing Paragraph Editing Menu action: '%s'", selected_option.get('desc'))
            try:
                action_func()
            except Exception:
                logger.critical(f"An unhandled error occurred while running '{selected_option.get('desc')}' in Paragraph Editing Menu.", exc_info=True)
    ...
    def _get_jobs_with_paragraphs_to_edit(self):
        ...
        try:
            with get_session() as session:
                ...
                logger.debug(f"Found {len(candidate_jobs_query)} candidate jobs from DB query for editing eligibility.")
                ...
                for job_data in candidate_jobs_query:
                    ...
                    if not paragraph_file_path.exists():
                        logger.warning(f"Paragraph JSON file for job {job_data.job_ulid} does not exist at {paragraph_file_path}. Marking as eligible for processing.")
                        self.console.print(f"[yellow]Warning: Paragraph JSON file for job {job_data.job_ulid} does not exist at {paragraph_file_path}. Marking as eligible for processing.[/yellow]")
                        ...
                        try:
                            ...
                            for entry in paragraphs_data:
                                if entry.get('edited') is None:
                                    needs_editing_processing = True
                                    logger.debug(f"Job {job_data.job_ulid} has unedited paragraphs.")
                                    break
                        except json.JSONDecodeError as e:
                            logger.error(f"Error decoding JSON for job {job_data.job_ulid} at {paragraph_file_path}: {e}. Marking as eligible.", exc_info=True)
                            self.console.print(f"[red]Error decoding JSON for job {job_data.job_ulid} at {paragraph_file_path}: {e}. Marking as eligible.[/red]")
                            needs_editing_processing = True
                        except Exception as e:
                            logger.error(f"Error reading paragraph JSON for job {job_data.job_ulid} at {paragraph_file_path}: {e}. Marking as eligible.", exc_info=True)
                            self.console.print(f"[red]Error reading paragraph JSON for job {job_data.job_ulid} at {paragraph_file_path}: {e}. Marking as eligible.[/red]")
                            needs_editing_processing = True
            ...
            logger.info(f"Identified {len(jobs_list)} jobs with paragraphs needing editing.")
        except Exception:
            logger.error("Error querying for jobs with paragraphs needing editing.", exc_info=True)
        return jobs_list
    ...
    def _select_and_process_single_edited_job(self):
        ...
        while True:
            ...
            logger.debug("User selected: '%s'", choice)
            if choice == 'b':
                logger.info("User opted to go back from single job paragraph editing selection.")
                return None
            
            selected_job_data = job_map.get(choice)
            if selected_job_data:
                self.console.print(f"[cyan]Selected Job:[/cyan] {selected_job_data['job_ulid']} - [dim]{selected_job_data['title']}[/dim]")
                logger.info(f"User selected Job ULID: {selected_job_data['job_ulid']} (ID: {selected_job_data['id']}) for single paragraph editing.")
                try:
                    editor_service = Editor(job_id=selected_job_data['id'])
                    editor_service.process_paragraphs_for_editing()
                    logger.info(f"Successfully processed paragraphs for Job ULID: {selected_job_data['job_ulid']}.")
                except Exception:
                    logger.error(f"Error processing paragraphs for Job ULID: {selected_job_data['job_ulid']}.", exc_info=True)
                self.console.input("Press Enter to continue...")
                return
            else:
                logger.warning("Invalid selection for single job paragraph editing: '%s'", choice)
                self.console.print("[red]Invalid selection. Please enter a valid number or 'b'.[/red]")
    ...
    def _process_all_edited_jobs(self):
        ...
        logger.info(f"Found {len(jobs_to_edit)} jobs with paragraphs to edit for bulk processing.")
        for i, job_data in enumerate(jobs_to_edit):
            self.console.print(f"[bold white]Processing Job ({i+1}/{len(jobs_to_edit)}):[/bold white] {job_data['job_ulid']} - {job_data['title']}")
            logger.info(f"Processing Job ({i+1}/{len(jobs_to_edit)}): Job ULID: {job_data['job_ulid']} (ID: {job_data['id']})")
            try:
                editor_service = Editor(job_id=job_data['id'])
                editor_service.process_paragraphs_for_editing()
                logger.info(f"Successfully processed paragraphs for Job ULID: {job_data['job_ulid']}.")
            except Exception:
                logger.error(f"Error processing paragraphs for Job ULID: {job_data['job_ulid']}.", exc_info=True)
            self.console.print()
    ...
    def run(self):
        ...
        while True:
            ...
            self.console.print(f"[cyan]Executing:[/cyan] {selected_option.get('desc')}")
            logger.info("Executing Editor Menu action: '%s'", selected_option.get('desc'))
            try:
                action_func()
            except Exception:
                logger.critical(f"An unhandled error occurred while running '{selected_option.get('desc')}' in Editor Menu.", exc_info=True)
                self.console.print(f"[bold red]An unexpected error occurred while running '{selected_option.get('desc')}'. Check logs for details.[/bold red]")
                self.console.input("Press Enter to continue...")
