        ...
        while True:
            ...
            if choice == "1":
                ...
                try:
                    ...
                    logger.critical("An unhandled error occurred during 'Format All Pending Transcriptions'.", exc_info=True)
                    self.console.print("[red]An unexpected error occurred. Check logs for details.[/red]")
            elif choice == "2":
                logger.info("User selected 'Format Selected Transcription'.")
                try:
                    self.format_selected_transcription()
                except Exception:
                    logger.critical("An unhandled error occurred during 'Format Selected Transcription'.", exc_info=True)
                    self.console.print("[red]An unexpected error occurred. Check logs for details.[/red]")
            elif choice == "b":
        ...
        with self.console.status(f"[bold green]Searching for job {job_id} to format...[/bold green]", spinner=config.SPINNER) as status:
            try:
                with get_session() as session:
                    ...
                    logger.info(f"Found job {job.job_ulid} (ID: {job.id}) ready for formatting.")
                    ...
                    if not whisper_stage or not whisper_stage.output_path or not Path(whisper_stage.output_path).exists():
                        logger.error(f"Missing transcribe_whisper output, output path, or file not found for job {job.job_ulid}.")
                        ...
                        if format_gemini_stage:
                            ...
                            session.commit()
                            logger.info(f"Updated format_gemini stage to FAILED for Job {job.job_ulid} due to missing whisper output.")
                        Prompt.ask("Press Enter to continue...")
                        return

                    if not format_gemini_stage:
                        logger.error(f"'format_gemini' stage not found for job {job.job_ulid}. Skipping.")
                        self.console.print(f"[bold red]Error: 'format_gemini' stage not found for job [cyan]{job.job_ulid}[/cyan]. Skipping.[/bold red]")
                        Prompt.ask("Press Enter to continue...")
                        return

                    try:
                        logger.info(f"Calling Formatter service for Job ULID: {job.job_ulid}.")
                        ...
                        if output_file_path is None:
                            logger.warning(f"Formatter returned None for job {job.job_ulid}. Marking as failed.")
                            ...
                            session.add(format_gemini_stage)
                            session.commit()
                            logger.info(f"Updated format_gemini stage to FAILED for Job {job.job_ulid} in DB.")
                        else:
                            format_gemini_stage.state = StageState.success
                            format_gemini_stage.output_path = str(output_file_path)
                            session.add(format_gemini_stage)
                            session.commit()
                            logger.info(f"Successfully formatted and saved for job {job.job_ulid}. Output path: {output_file_path}.")
                            self.console.print(f"[bold green]Successfully formatted and saved for job [cyan]{job.job_ulid}[/cyan].[/bold green]")

                    except RuntimeError as e:
                        logger.error(f"RuntimeError during formatting for job {job.job_ulid}: {e}", exc_info=True)
                        ...
                        session.add(format_gemini_stage)
                        session.commit()
                        logger.info(f"Updated format_gemini stage to FAILED for Job {job.job_ulid} in DB due to RuntimeError.")
                    except Exception as e:
                        logger.critical(f"An unexpected error occurred during formatting for job {job.job_ulid}: {e}", exc_info=True)
                        self.console.print(f"[bold red]An unexpected error occurred for job [cyan]{job.job_ulid}[/cyan]: {e}[/bold red]")
                        format_gemini_stage.state = StageState.failed
                        format_gemini_stage.last_error = str(e)
                        session.add(format_gemini_stage)
                        session.commit()
                        logger.info(f"Updated format_gemini stage to FAILED for Job {job.job_ulid} in DB due to unexpected error.")
                
            except Exception as e:
                logger.critical(f"An error occurred during transcription formatting selection process: {e}", exc_info=True)
        ...
        with self.console.status("[bold green]Searching for transcriptions to format...[/bold green]", spinner=config.SPINNER) as status:
            try:
                with get_session() as session:
                    ...
                    logger.info(f"Found {len(jobs_to_format)} transcriptions to format.")
                    ...
                    for i, job in enumerate(jobs_to_format):
                        ...
                        logger.info(f"Processing transcription {i + 1}/{len(jobs_to_format)} for Job ULID: {job.job_ulid} (ID: {job.id}).")
                        
                        whisper_stage = next((s for s in job.stages if s.stage_name == "transcribe_whisper"), None)
                        format_gemini_stage = next((s for s in job.stages if s.stage_name == "format_gemini"), None)

                        if not whisper_stage or not whisper_stage.output_path or not Path(whisper_stage.output_path).exists():
                            logger.error(f"Missing transcribe_whisper output, output path, or file not found for job {job.job_ulid}. Skipping.")
                            self.console.print(f"[bold red]Error: No successful 'transcribe_whisper' stage, output path missing, or transcript file not found for job [cyan]{job.job_ulid}[/cyan]. Skipping.[/bold red]")
                            if format_gemini_stage:
                                format_gemini_stage.state = StageState.failed
                                format_gemini_stage.last_error = "Missing transcribe_whisper output or file not found."
                ...
                logger.critical(f"An error occurred during the 'Format All Pending Transcriptions' process: {e}", exc_info=True)
                status.update(f"[bold red]An error occurred during transcription formatting: {e}[/bold red]")
                self.console.print(f"[bold red]An error occurred during transcription formatting: {e}[/bold red]")
