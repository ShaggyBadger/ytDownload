...
def _get_jobs_for_metadata_processing():
    ...
    try:
        with get_session() as session:
            ...
            candidate_jobs_query = session.query(
            ...
            ).join(JobStage, JobInfo.id == JobStage.job_id).filter(
                JobInfo.id == successful_format_gemini_jobs.c.job_id,
                JobStage.stage_name == "extract_metadata"
            ).distinct().all()
            logger.debug(f"Found {len(candidate_jobs_query)} candidate jobs for metadata processing.")
            ...
            for job_data in candidate_jobs_query:
                ...
                if not metadata_path.exists():
                    logger.info(f"Metadata file not found at {metadata_path} for job {job_data.job_ulid}. Marking as eligible.")
                    ...
                    try:
                        ...
                        for category in config.METADATA_CATEGORIES:
                            if metadata.get(category) is None:
                                logger.debug(f"Job {job_data.job_ulid} metadata '{category}' is null. Marking as eligible.")
                                needs_processing = True
                                break
                    except json.JSONDecodeError:
                        logger.error(f"Corrupt metadata.json for job {job_data.job_ulid} at {metadata_path}. Marking as eligible.", exc_info=True)
                        console.print(f"[red]Warning: Corrupt metadata.json for job {job_data.job_ulid} at {metadata_path}. Marking as eligible.[/red]")
                        needs_processing = True
                    except Exception:
                        logger.error(f"Error checking metadata.json for job {job_data.job_ulid} at {metadata_path}.", exc_info=True)
                        console.print(f"[red]Error reading metadata.json for job {job_data.job_ulid} at {metadata_path}. Marking as eligible.[/red]")
                        needs_processing = True
        ...
        logger.info(f"Identified {len(eligible_jobs)} jobs eligible for metadata processing.")
    except Exception:
        logger.error("Error querying for jobs eligible for metadata processing.", exc_info=True)
...
def _run_metadata_processing(job_ids: list[int]):
    ...
    if not job_ids:
        ...
        console.print("[bold yellow]No jobs selected for metadata processing.[/bold yellow]")
        return
    ...
    for job_id in job_ids:
        ...
        try:
            ...
            logger.info(f"Successfully finished processing metadata for Job ID: {job_id}.")
            console.print(f"[bold green]Finished processing metadata for Job ID: {job_id}.[/bold green]\n")
        except Exception:
            logger.error(f"Error processing metadata for Job ID: {job_id}.", exc_info=True)
            console.print(f"[bold red]Error processing metadata for Job ID: {job_id}. Check logs.[/bold red]\n")
...
def metadata_generator_menu():
    ...
    while True:
        ...
        if choice == '1':
            ...
            console.print("[bold blue]Fetching eligible jobs for processing...[/bold blue]")
            jobs_to_run = _get_jobs_for_metadata_processing()
            _display_jobs(jobs_to_run, title="Eligible Jobs for Automatic Metadata Processing")
            if jobs_to_run:
                confirm = Prompt.ask("[bold yellow]Confirm processing metadata for these jobs? (y/n):[/bold yellow] ").lower()
                logger.debug("User confirmation for processing all eligible jobs: '%s'", confirm)
                if confirm == 'y':
                    _run_metadata_processing([job['id'] for job in jobs_to_run])
                else:
                    logger.info("Metadata processing for all eligible jobs cancelled by user.")
            ...
            jobs_for_selection = _get_jobs_for_metadata_processing() # Use the same method for eligibility
            _display_jobs(jobs_for_selection, title="Jobs with Metadata to Process")

            if jobs_for_selection:
                while True:
                    selected_ids_input = Prompt.ask("[bold]Enter Job IDs to process (comma-separated), or 'b' to go back:[/bold] ")
                    logger.debug("User entered selected IDs for manual processing: '%s'", selected_ids_input)
                    ...
                    try:
                        ...
                        if valid_selected_ids:
                            ...
                            logger.warning("No valid Job IDs entered or selected IDs are not in the list: %s", selected_ids_input)
                            console.print("[bold red]No valid Job IDs entered or selected IDs are not in the list. Please try again.[/bold red]")
                    except ValueError:
                        logger.error("Invalid input for Job IDs (not comma-separated numbers): '%s'", selected_ids_input, exc_info=True)
            ...
            logger.info("Exiting Metadata Generator Menu. User selected 'Back to Main Menu'.")
            break
        else:
